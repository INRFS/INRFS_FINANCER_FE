...
export default function Financers() {
  ...
  const loadFinancers = React.useCallback(async () => {
    try {
      const payload = await platformApi.admin.allFinancers();
      setFinancers(pageItems(payload).map((item) => ({ ...item, name: item.displayName, owner: item.ownerName })));
    } catch (error) {
      setPageError(error.message);
    }
  }, []);
  useEffect(() => { loadFinancers(); }, [loadFinancers]);
  ...
  const changeStatus = async (id, status) => {
    try {
      const updated = await platformApi.admin.changeFinancerStatus(id, { status, reason: `Status changed to ${status} by administrator` });
      setFinancers((current) => current.map((item) => item.id === id ? { ...item, ...updated, name: updated.displayName, owner: updated.ownerName } : item));
    } catch (error) { setPageError(error.message); }
  };

  const handleApprove = (id) => changeStatus(id, 'Active');

  const handleActivate = (id) => changeStatus(id, 'Active');

  const handleDeactivate = (id) => changeStatus(id, 'Inactive');

  const handleSuspend = (id) => changeStatus(id, 'Suspended');
  ...
  const handleAddSubmit = async (event) => {
    ...
    try {
      await platformApi.admin.createFinancer({ legalName: newFinancer.name, displayName: newFinancer.name, ownerName: newFinancer.owner, email: newFinancer.email, phone: newFinancer.phone, addressLine: newFinancer.addressLine, city: newFinancer.city, state: newFinancer.state, postalCode: newFinancer.postalCode, taxNumber: null, registrationNumber: null, serviceChargePercentage: Number(newFinancer.serviceChargePercentage) });
      setIsAddModalOpen(false);
      setNewFinancer({ name: '', owner: '', city: 'Ahmedabad', state: 'Gujarat', email: '', phone: '', addressLine: '', postalCode: '', serviceChargePercentage: DEFAULT_SERVICE_CHARGE_PERCENTAGE });
      await loadFinancers();
    } catch (error) { setPageError(error.message); }
  };
      ...
      >
        ...
        >
          ...
          {[
            ...
            ['addressLine', 'Business Address', 'text'],
            ['state', 'State', 'text'],
            ['postalCode', 'Postal Code', 'text'],
          ].map(([field, label, type]) => (
            <div className="financer-register-field" key={field}>
              <label htmlFor={`financer-${field}`}>{label}</label>
              <input
                id={`financer-${field}`}
                type={type}
                value={newFinancer[field]}
                onChange={(event) => setNewFinancer({ ...newFinancer, [field]: event.target.value })}
                required
              />
            </div>
          ))}

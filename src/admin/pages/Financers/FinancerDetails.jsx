...
export default function FinancerDetails() {
   ...
   *
   * GET /api/admin/financers/:financerId
   */

  const financer = useMemo(() => {
    return mockFinancersList.find(
      (item) => item.id === financerId
    );
  }, [financerId]);
    ...
    return (
      <div className="financer-details-page">
        <div className="financer-details-not-found">
          <div className="financer-not-found-icon">
            <Building2 size={28} />
          </div>
          ...
          <p>
            The financer account you're looking for
  ...
  const serviceCharge =
    calculateServiceCharge(financer);

  const customers = Array.isArray(financer.customers)
    ? financer.customers
    : [];

  const loans = Array.isArray(financer.loans)
    ? financer.loans
    : [];

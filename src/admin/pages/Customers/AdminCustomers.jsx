...
export default function AdminCustomers() {
  ...
  useEffect(() => {
    let current = true;
    Promise.all([platformApi.customers.all(), platformApi.admin.allFinancers()])
      .then(([customerPayload, financerPayload]) => {
        if (!current) return;
        const financers = new Map(pageItems(financerPayload).map((item) => [item.id, item]));
        setCustomers(pageItems(customerPayload).map((item) => ({ ...customerFromApi(item), financerName: financers.get(item.financerId)?.displayName || item.financerId })));
      })

...
export default function AdminSMSManagement() {
  ...
  const [financerUsage, setFinancerUsage] = useState([]);
  const [smsActivity] = useState([]);
  const [pageError, setPageError] = useState('');
  useEffect(() => {
    Promise.all([platformApi.admin.smsUsage({}), platformApi.admin.allFinancers()]).then(([usage, financerPayload]) => {
      const financers = new Map(pageItems(financerPayload).map((item) => [item.id, item]));
      setStats({ sentThisMonth: usage.summary?.sent, delivered: usage.summary?.delivered, failed: usage.summary?.failed, platformCreditLimit: Math.max(usage.summary?.credits || 0, 1) });
      const grouped = new Map();
      (usage.byFinancer || []).forEach((item) => { const current = grouped.get(item.financerId) || { financerId: item.financerId, financer: financers.get(item.financerId)?.displayName || item.financerId, used: 0, limit: 0 }; current.used += Number(item.credits || 0); current.limit = Math.max(current.used, 1); grouped.set(item.financerId, current); });
      setFinancerUsage([...grouped.values()].map((item) => ({ ...item, usage: item.limit ? item.used / item.limit * 100 : 0 })));
            ...
            <div className="inrfs-sms-modal-section">
              ...
              <div className="inrfs-sms-message-preview">
                {selectedActivity.message ||
                  'Message content was not included in the API response.'}
              </div>
            </div>

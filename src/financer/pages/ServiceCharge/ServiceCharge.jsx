...
const serviceChargeHistory = [
  {
    month: 'August 2026',
    interestCollected: 28500,
    chargeRate: 1,
    amountPayable: 285,
    amountPaid: 0,
    outstanding: 285,
...
export default function ServiceCharge() {
  ...
  const currentBilling = serviceChargeHistory[0];

  const totals = useMemo(() => {
    const totalPayable = serviceChargeHistory.reduce(
      (sum, item) => sum + item.amountPayable,
      0
    );

    const totalPaid = serviceChargeHistory.reduce(
  ...
  const handleContactOperations = () => {
    alert('Operations team contact request submitted.');
  };

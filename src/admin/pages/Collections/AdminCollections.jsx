...
const AdminCollections = () => {
  ...
  useEffect(() => { platformApi.collections.list({ pageSize: 100 }).then((payload) => setRecords(pageItems(payload).map((item) => ({ ...item, collectionId: item.id, customerName: item.customer, loanId: item.loanNumber, expectedAmount: item.due, outstandingAmount: item.due, collectionStatus: item.daysPastDue > 0 ? 'Overdue' : 'Pending' })))).catch((error) => setPageError(error.message)); }, []);
  const collectionRecords = useMemo(() => normalizeCollections(records), [records]);

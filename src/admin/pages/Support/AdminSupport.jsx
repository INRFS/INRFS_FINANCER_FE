...
export default function AdminSupport() {
  ...
  return <div className="admin-support-page animate-fade-in">
    ...
    <section className="admin-support-toolbar" aria-label="Ticket filters"><label><Search size={16} /><span className="sr-only">Search tickets</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search number, subject, category..." /></label><label><Filter size={16} /><span className="sr-only">Filter by status</span><select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}><option value="">All statuses</option><option>Open</option><option>InProgress</option><option>Resolved</option><option>Closed</option></select></label></section>
